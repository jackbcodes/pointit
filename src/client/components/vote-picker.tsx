    ...
    >
      {game.votingSystem.values.map((value) => (
        <ToggleGroupPrimitive.Item
          key={value}
          value={value}
          className="relative flex h-20 w-14 items-center justify-center rounded-lg border-4 border-primary text-primary transition-all hover:opacity-90 disabled:pointer-events-none disabled:opacity-50 data-[state=on]:bg-primary data-[state=disabled]:text-primary-foreground data-[state=on]:text-primary-foreground"
        >
          <SuitIcon
            vote={value}
